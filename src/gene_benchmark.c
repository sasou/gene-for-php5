 ...
 */
void markStart() {
	zval *func = NULL, *ret = NULL;
	gettimeofday( &start, NULL );

	MAKE_STD_ZVAL(func);
	ZVAL_STRING(func, "memory_get_peak_usage", 1);
	MAKE_STD_ZVAL(ret);
	call_user_function(EG(function_table), NULL, func, ret, 0, NULL);
	if (Z_TYPE_P(ret) == IS_LONG) {
		memory_start = Z_LVAL_P(ret);
	}
	zval_ptr_dtor(&func);
	if (ret) {
		zval_ptr_dtor(&ret);
...
GENE_MINIT_FUNCTION(benchmark)
{
    ...
    gene_benchmark_ce = zend_register_internal_class(&gene_benchmark TSRMLS_CC);
