...
zval *gene_reg_instance(zval *this_ptr TSRMLS_DC) {
	zval *instance = zend_read_static_property(gene_reg_ce,
			GENE_REG_PROPERTY_INSTANCE, strlen(GENE_REG_PROPERTY_INSTANCE),
			1 TSRMLS_CC);

	if (Z_TYPE_P(instance) != IS_OBJECT
			|| !instanceof_function(Z_OBJCE_P(instance),
					gene_reg_ce TSRMLS_CC)) {
		zval *regs;
		...
		MAKE_STD_ZVAL(regs);
		array_init(regs);
		zend_update_property(gene_reg_ce, instance, GENE_REG_PROPERTY_REG,
				strlen(GENE_REG_PROPERTY_REG), regs TSRMLS_CC);
		zend_update_static_property(gene_reg_ce, GENE_REG_PROPERTY_INSTANCE,
				strlen(GENE_REG_PROPERTY_INSTANCE), instance TSRMLS_CC);
		zval_ptr_dtor(&regs);
...
PHP_METHOD(gene_reg, get) {
	...
	entrys = zend_read_property(gene_reg_ce, reg, GENE_REG_PROPERTY_REG,
			strlen(GENE_REG_PROPERTY_REG), 1 TSRMLS_CC);
	if (entrys && Z_TYPE_P(entrys) == IS_ARRAY) {
		if (zend_hash_find(Z_ARRVAL_P(entrys), name, len + 1,
				(void **) &ppzval) == SUCCESS) {
...
PHP_METHOD(gene_reg, set) {
	...
	char *name;
	int len;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sz", &name, &len,
			&value) == FAILURE) {
		RETURN_NULL()
		;
	}
	...
	entrys = zend_read_property(gene_reg_ce, reg, GENE_REG_PROPERTY_REG,
			strlen(GENE_REG_PROPERTY_REG), 1 TSRMLS_CC);
	Z_ADDREF_P(value);
	if (zend_hash_update(Z_ARRVAL_P(entrys), name, len + 1, &value,
...
GENE_MINIT_FUNCTION(reg) {
	...
	//static
	zend_declare_property_null(gene_reg_ce, GENE_REG_PROPERTY_INSTANCE,
			strlen(GENE_REG_PROPERTY_INSTANCE),
			ZEND_ACC_PROTECTED | ZEND_ACC_STATIC TSRMLS_CC);
	zend_declare_property_null(gene_reg_ce, GENE_REG_PROPERTY_REG,
			strlen(GENE_REG_PROPERTY_REG), ZEND_ACC_PROTECTED TSRMLS_CC);
	//
	return SUCCESS;
}
/* }}} */
