...
PHP_METHOD(gene_config, set)
{
	...
	int keyString_len,validity = 0,router_e_len,pathlen;
	zval *zvalue,*safe;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sz|l", &keyString, &keyString_len, &zvalue, &validity) == FAILURE) {
		return;
	}
	safe = zend_read_property(gene_config_ce, getThis(), GENE_CONFIG_SAFE, strlen(GENE_CONFIG_SAFE), 1 TSRMLS_CC);
	if (Z_STRLEN_P(safe)) {
	...
	if (zvalue) {
		pathlen = spprintf(&path, 0, "%s", keyString);
		replaceAll(path,'.','/');
...
PHP_METHOD(gene_config, get)
{
	...
	int router_e_len,keyString_len,pathlen;
	char *router_e,*keyString,*path;
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s", &keyString, &keyString_len) == FAILURE) {
		return;
	}
	safe = zend_read_property(gene_config_ce, self, GENE_CONFIG_SAFE, strlen(GENE_CONFIG_SAFE), 1 TSRMLS_CC);
	if (Z_STRLEN_P(safe)) {
		router_e_len = spprintf(&router_e, 0, "%s%s", Z_STRVAL_P(safe), GENE_CONFIG_CACHE);
	} else {
		router_e_len = spprintf(&router_e, 0, "%s", GENE_CONFIG_CACHE);
...
GENE_MINIT_FUNCTION(config)
{
    ...
    INIT_CLASS_ENTRY(gene_config,"gene_config",gene_config_methods);
