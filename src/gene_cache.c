	...
	if (zvalue) {
		...
		switch (ret->type) {
			...
			case IS_STRING:
					...
					ret->value.str.val = str_add(zvalue->value.str.val, zvalue->value.str.len);
				break;
	#ifdef IS_CONSTANT_ARRAY
			case IS_CONSTANT_ARRAY:
	#endif
			case IS_ARRAY: {
					...
					zend_hash_init(tmp_ht, zend_hash_num_elements(original_ht), NULL, (dtor_func_t) gene_cache_zval_dtor, 1);
 ...
 */
static void * gene_cache_zval_edit_persistent(zval *copyval,zval *zvalue TSRMLS_DC) {
	if (!zvalue) return NULL;
	switch (Z_TYPE_P(copyval)) {
		case IS_STRING:
			if ((Z_TYPE_P(zvalue) != IS_STRING) && (copyval->value.str.val != NULL)) {
	...
	switch (Z_TYPE_P(zvalue)) {
		...
		case IS_STRING:
				...
				if (Z_TYPE_P(copyval) != IS_STRING) {
					copyval->value.str.val = str_add(zvalue->value.str.val, zvalue->value.str.len);
				} else {
					copyval->value.str.val = r_strndup(copyval->value.str.val,zvalue->value.str.val, zvalue->value.str.len);
				}
		...
		case IS_ARRAY: {
				if (Z_TYPE_P(copyval) != IS_ARRAY) {
					...
					zend_hash_init(tmp_ht, zend_hash_num_elements(zvalue->value.ht), NULL,(dtor_func_t) gene_cache_zval_dtor, 1);
					gene_cache_copy_persistent(tmp_ht, zvalue->value.ht TSRMLS_CC);
					copyval->value.ht = tmp_ht;
				} else {
					gene_cache_copy_persistent(copyval->value.ht, zvalue->value.ht TSRMLS_CC);
				}
...
static void gene_cache_copy_persistent(HashTable *pdst, HashTable *src TSRMLS_DC) {
	...
	long idx;
	for(zend_hash_internal_pointer_reset(pdst);zend_hash_has_more_elements(pdst) == SUCCESS;zend_hash_move_forward(pdst)) {
		if (zend_hash_get_current_key_ex(pdst, &key, &keylen, &idx, 0, NULL) == HASH_KEY_IS_LONG) {
	...
	for(zend_hash_internal_pointer_reset(src);zend_hash_has_more_elements(src) == SUCCESS;zend_hash_move_forward(src)) {
		if (zend_hash_get_current_key_ex(src, &key, &keylen, &idx, 0, NULL) == HASH_KEY_IS_LONG) {
			...
			if( zend_hash_find(pdst,key, keylen+1,(void**)&copyval) == FAILURE )
			{
				tmp = gene_cache_zval_persistent(*ppzval TSRMLS_CC);
				if (tmp) {
					keyvallen = spprintf(&keyval, 0, "%s", key);
					zend_hash_update(pdst, keyval, keyvallen+1, (void **)&tmp, sizeof(zval *), NULL);
					efree(keyval);
					keyval = NULL;
				}
			} else {
...
void gene_cache_set(char *keyString, int keyString_len, zval *zvalue, int validity TSRMLS_DC)
{
	...
	if (zvalue) {
		if (zend_hash_find(GENE_G(cache), keyString, keyString_len+1, (void **)&copyval) == FAILURE){
			ret = (gene_cache_container *) pemalloc(sizeof(gene_cache_container), 1);
			...
			zend_hash_add(GENE_G(cache),key, len+1, &ret, sizeof(gene_cache_container *), NULL);
			efree(key);
			key = NULL;
			return;
		}
...
zval * gene_cache_get(char *keyString, int keyString_len TSRMLS_DC)
{
	...
	if (zend_hash_find(GENE_G(cache), keyString, keyString_len+1,(void **)&zvalue) == SUCCESS){
...
zval * gene_params(char *keyString, int keyString_len TSRMLS_DC)
{
	...
	if (keyString != NULL && zend_hash_find(GENE_G(params), keyString, keyString_len+1, (void **)&ret) == SUCCESS){
 ...
 */
void gene_cache_set_easy(char *keyString, int keyString_len, int times, int validity TSRMLS_DC)
{
	char *key;
	int len;
	gene_cache_container_easy **val,*ret;
	if (zend_hash_find(GENE_G(cache_easy), keyString, keyString_len+1, (void **)&val) == FAILURE){
		ret = (gene_cache_container_easy *) pemalloc(sizeof(gene_cache_container_easy), 1);
		ret->stime = time(NULL);
		ret->validity = validity;
		ret->status = 0;
		ret->ftime = times;
		len = spprintf(&key, 0, "%s", keyString);
		zend_hash_add(GENE_G(cache_easy),keyString, keyString_len+1, &ret, sizeof(gene_cache_container_easy *), NULL);
		efree(key);
		key = NULL;
		return;
	}
...
gene_cache_container_easy * gene_cache_get_easy(char *keyString, int keyString_len TSRMLS_DC)
{
	...
	if (zend_hash_find(GENE_G(cache_easy), keyString, keyString_len+1,(void **)&zvalue) == SUCCESS){
		return (*zvalue);
	}
	return NULL;
}
	...
	if (zend_hash_find(GENE_G(cache), keyString, keyString_len+1, (void **)&copyval) == FAILURE){
		...
		while (seg) {
			if (ptr && strlen(ptr)>0) {
				...
				val = gene_cache_set_val(tmp, seg, strlen(seg), zvalue TSRMLS_CC);
			}
			tmp = val;
			seg = php_strtok_r(NULL, "/", &ptr);
		}
		...
		if ((*copyval)->status == 0) {
			...
			while (seg) {
				if (ptr && strlen(ptr)>0) {
					tmp = gene_cache_set_val(tmp, seg, strlen(seg), NULL TSRMLS_CC);
				} else {
					tmp = gene_cache_set_val(tmp, seg, strlen(seg), zvalue TSRMLS_CC);
				}
				seg = php_strtok_r(NULL, "/", &ptr);
			}
			(*copyval)->status = 0;
...
zval* gene_cache_set_val(zval *val, char *keyString, int keyString_len, zval *zvalue TSRMLS_DC)
{
	...
	int len;
	zval *tmp,**copyval;
	if (zend_hash_find(Z_ARRVAL_P(val), keyString, keyString_len+1, (void **)&copyval) == FAILURE){
		if (zvalue){
			tmp = gene_cache_zval_persistent(zvalue TSRMLS_CC);
		} else {
			tmp = gene_cache_zval_persistent(NULL TSRMLS_CC);
		...
		len = spprintf(&key, 0, "%s", keyString);
		zend_hash_add(Z_ARRVAL_P(val),key, len+1, (void **)&tmp, sizeof(zval *), NULL);
		efree(key);
	} else {
		if (zvalue){
...
{
	gene_cache_container **zvalue;
	if (zend_hash_find(GENE_G(cache), keyString, keyString_len+1,(void **)&zvalue) == FAILURE){
		return 0;
	}
	return (*zvalue)->ctime;
}
...
PHP_METHOD(gene_cache, __construct)
{
    ...
    if(safe) {
    	zend_update_property_string(gene_cache_ce, getThis(), GENE_CACHE_SAFE, strlen(GENE_CACHE_SAFE), Z_STRVAL_P(safe) TSRMLS_CC);
    } else {
    	if (GENE_G(app_key)) {
...
PHP_METHOD(gene_cache, set)
{
	...
	safe = zend_read_property(gene_cache_ce, getThis(), GENE_CACHE_SAFE, strlen(GENE_CACHE_SAFE), 1 TSRMLS_CC);
	if (Z_STRLEN_P(safe)) {
...
GENE_MINIT_FUNCTION(cache)
{
	...
	//debug
    zend_declare_property_string(gene_cache_ce, GENE_CACHE_SAFE, strlen(GENE_CACHE_SAFE), "", ZEND_ACC_PUBLIC TSRMLS_CC);
    //
	return SUCCESS;
}
/* }}} */
